...
int main()
{
	...
	int dstn = open("ex1.memcpy.txt", O_RDWR | O_CREAT, (mode_t)0777);
	if (dstn == -1)
	{
    ...
    void *out = mmap(NULL, srcsize, PROT_WRITE | PROT_READ, MAP_SHARED, dstn, 0);
  	...
  	if (out == MAP_FAILED)
	{
  	...
  	int unmap_dstn = munmap(out, srcsize);
  	if (unmap_dstn != 0)
  	{
  		close(dstn);
    	perror("Could not munmap the file\n");
